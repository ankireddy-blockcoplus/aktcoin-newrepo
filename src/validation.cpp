...
#if defined(NDEBUG)
# error "Aktcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Aktcoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s AKT additional fees, %d delta bytes\n",
